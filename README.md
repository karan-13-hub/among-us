...
<img src="XXXX" alt="Among Us" width="400"/>
...
main.py
```
You will need to add a `.env` file with an [OpenRouter](XXXX) API key.

Alternatively, you can download 400 full-game logs (for `Phi-4-15b` and `Llama-3.3-70b-instruct`) and 810 game summaries from the [HuggingFace](XXXX) dataset to reproduce the results in the paper (and evaluate your own techniques!).
...
Alternatively, you can download the ground truth labels from the [HuggingFace](XXXX).
...
We use the [Goodfire API](XXXX) to evaluate SAE features on the game logs. To do this, run the notebook:
...
- Our game logic uses a bunch of code from [AmongAgents](XXXX).

If you face any bugs or issues with this codebase, please contact XXXX-2 XXXX-3 (XXXX-1) at XXXX-8@gmail.com.
